...
f_Palma_complemento<-function(directorio,mes,anio){
  ...
  archivo=nombre_archivos[nombre_archivos$PRODUCTO=="Precio_Palma","NOMBRE"]

  # Especifica la ruta del archivo de Excel
  Precio_interno <- read.xlsx(paste0(directorio,"/ISE/",anio,"/",carpeta,"/Data/consolidado_ISE/Palma/",archivo))
  ...
  Precio=as.data.frame(Precio_interno[fila[1]:(fila[1]+tamaño-1),columna1[2]])
...
#precio internacional
  ...
  columna1=which(grepl("Palm oil",Precio_internacional),arr.ind = TRUE)
...
#IPC
  ...
  fila1=which(grepl("01150100",IPC[,1]),arr.ind = TRUE)
...
#IPP
  ...
  fila1=which(grepl("21535",IPP[,n_col]),arr.ind = TRUE)
...
#EMMET
  ...
  Palma <- read.xlsx(paste0(directorio,"/ISE/",anio,"/",carpeta,"/Data/consolidado_ISE/EMMET/",archivo),
                      sheet = "COMPLETO")
  # Seleccionar solo las columnas que necesitas
  Palma_tabla <- Palma[, c("anio", "mes", "EMMET_Clase", "ProduccionRealPond")]
  Palma_tabla=Palma_tabla %>%
    filter(EMMET_Clase==1030 )%>%
    group_by(anio,mes)%>%
    filter(anio>(anio-3)) %>%
    summarise(suma=sum(ProduccionRealPond))%>%
    as.data.frame()
  tamaño=36+mes
  Palma_tabla=tail(Palma_tabla$suma,tamaño)
  var_anual=Palma_tabla/lag(Palma_tabla,12)*100-100
  Palma_tabla_ant=lag(Palma_tabla,12)
  tamaño=length(Palma_tabla)
  Estado <- rep("",tamaño)

  for (i in seq(3, tamaño, by = 3)) {
    Estado[i] <- (sum(Palma_tabla[(i-2):i]) / sum(Palma_tabla_ant[(i-2):i]))*100-100  # Realiza la suma y división
  }
  Estado=as.numeric(Estado)
  Observaciones <- rep("",tamaño)

  for (i in seq(12, tamaño, by = 12)) {
    Observaciones[i] <- (sum(Palma_tabla[(i-11):i]) / sum(Palma_tabla_ant[(i-11):i]))*100-100  # Realiza la suma y división
  }
  Observaciones=as.numeric(Observaciones)
  ...
  nuevos_datos=bind_rows(cuadro_precio,cuadro_precio_int,cuadro_Valor_IPC,cuadro_Valor_IPP,cuadro_EMMET)
  return(nuevos_datos)
}
